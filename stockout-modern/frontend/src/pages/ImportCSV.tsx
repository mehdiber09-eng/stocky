...
export default function ImportCSV() {
  ...
  async function importAll() {
    ...
    setToast({ msg: `${success}/${updated.length} ventes importées`, type: success > 0 ? 'success' : 'error' })
  }
    ...
    <div className="animate-fade-in max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-zinc-100">Import CSV</h1>
            <p className="text-sm text-zinc-500">Importez vos ventes en masse</p>
          </div>
        </div>
      ...
      >
        ...
        <p className="text-zinc-400 font-medium mb-1">Glissez votre fichier CSV ici</p>
        <p className="text-zinc-600 text-sm">ou cliquez pour parcourir</p>
        <input ref={fileRef} type="file" accept=".csv" className="hidden" onChange={handleFile} />
      </div>

      {/* Format info */}
      <div className="bg-surface-tertiary rounded-lg px-4 py-3 text-xs text-zinc-500">
        <p className="font-medium text-zinc-400 mb-1">Format attendu :</p>
      ...
      {rows.length > 0 && (
        <div className="card p-0 overflow-hidden">
          <div className="px-6 py-4 border-b border-surface-border flex items-center justify-between">
            <h2 className="font-medium text-zinc-100">{rows.length} lignes détectées</h2>
            <div className="flex items-center gap-3">
              {success > 0 && <span className="text-xs text-emerald-400">{success} importées</span>}
              {errors > 0 && <span className="text-xs text-red-400">{errors} erreurs</span>}
              <button
                onClick={importAll}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-border">
                <th className="text-left px-6 py-3 text-xs font-medium text-zinc-500 uppercase">ID Produit</th>
                <th className="text-left px-6 py-3 text-xs font-medium text-zinc-500 uppercase">Quantité</th>
                <th className="text-left px-6 py-3 text-xs font-medium text-zinc-500 uppercase">Statut</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row, i) => (
                <tr key={i} className="border-b border-surface-border last:border-0 hover:bg-surface-tertiary/50">
                  ...
                  <td className="px-6 py-3">
                    {row.status === 'pending' && <span className="text-zinc-600 text-xs">En attente</span>}
                    {row.status === 'success' && <span className="flex items-center gap-1 text-emerald-400 text-xs"><CheckCircle size={12} /> Importé</span>}
