import React, { useEffect, useState } from 'react'
import { Link, Navigate } from 'react-router-dom'
import { Activity, TrendingUp, BarChart2, Bell, Shield, Zap, ArrowRight, CheckCircle, MessageCircle, Upload, GitCompare } from 'lucide-react'
import { useAuth } from '../context/AuthContext'

const features = [
  { icon: TrendingUp, title: 'Prédiction ML', desc: 'XGBoost + LSTM pour anticiper vos ruptures avec précision et intervalle de confiance à 95%', color: 'text-brand-400 bg-brand-500/10' },
  { icon: BarChart2, title: 'Analytics avancés', desc: 'Historique complet, graphiques interactifs et export CSV de toutes vos données', color: 'text-emerald-400 bg-emerald-500/10' },
  { icon: Bell, title: 'Alertes email', desc: 'Notification automatique quand le risque dépasse votre seuil personnalisé', color: 'text-amber-400 bg-amber-500/10' },
  { icon: GitCompare, title: 'Comparaison', desc: 'Comparez jusqu\'à 5 produits simultanément avec classement par niveau de risque', color: 'text-purple-400 bg-purple-500/10' },
  { icon: Upload, title: 'Import CSV', desc: 'Importez vos ventes en masse par drag & drop avec validation ligne par ligne', color: 'text-pink-400 bg-pink-500/10' },
  { icon: MessageCircle, title: 'Conseiller IA', desc: 'Chatbot expert en supply chain pour vous guider dans vos décisions de stock', color: 'text-cyan-400 bg-cyan-500/10' },
]

const steps = [
  { n: '01', title: 'Créez vos produits', desc: 'Ajoutez votre catalogue avec SKU, délai de livraison et stock de sécurité' },
  { n: '02', title: 'Enregistrez vos ventes', desc: 'Importez en masse via CSV ou saisissez manuellement vos mouvements de stock' },
  { n: '03', title: 'Lancez une prédiction', desc: 'Obtenez en 1 clic la probabilité de rupture sur l\'horizon de votre choix' },
  { n: '04', title: 'Recevez des alertes', desc: 'Soyez notifié par email dès qu\'un produit dépasse le seuil de risque' },
]

const plans = [
  { name: 'Gratuit', price: '0€', period: '', features: ['5 prédictions', 'Tous les produits', 'Export CSV', 'Conseiller IA', 'Support email'], cta: 'Commencer gratuitement', highlight: false },
  { name: 'Pro', price: '9€', period: '/mois', features: ['Prédictions illimitées', 'Alertes email auto', 'Analytics avancés', 'Import CSV en masse', 'Comparaison multi-produits', 'Support prioritaire'], cta: 'Essayer 14 jours gratuits', highlight: true },
]

const stats = [
  { value: '95%', label: 'Précision ML' },
  { value: '<1s', label: 'Temps de réponse' },
  { value: '∞', label: 'Produits supportés' },
  { value: '24/7', label: 'Monitoring' },
]

export default function Landing() {
  const { isAuthenticated } = useAuth()
  const [scrolled, setScrolled] = useState(false)

  useEffect(() => {
    const fn = () => setScrolled(window.scrollY > 20)
    window.addEventListener('scroll', fn)
    return () => window.removeEventListener('scroll', fn)
  }, [])

  if (isAuthenticated || !!localStorage.getItem('token')) {
    return <Navigate to="/dashboard" replace />
  }

  return (
    <div className="min-h-screen bg-surface text-zinc-100">
      {/* Nav */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-surface/90 backdrop-blur border-b border-surface-border' : ''}`}>
        <div className="max-w-6xl mx-auto px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-brand-600 flex items-center justify-center">
              <Activity size={16} className="text-white" />
            </div>
            <span className="font-semibold text-zinc-100">StockSense</span>
          </div>
          <div className="flex items-center gap-3">
            <Link to="/login" className="btn-ghost text-sm">Connexion</Link>
            <Link to="/register">
              <button className="btn-primary text-sm">Démarrer gratuitement</button>
            </Link>
          </div>
        </div>
      </nav>

      {/* Hero */}
      <section className="max-w-6xl mx-auto px-8 pt-40 pb-24 text-center">
        <div className="inline-flex items-center gap-2 bg-brand-500/10 border border-brand-500/20 rounded-full px-4 py-1.5 text-xs text-brand-400 mb-8 animate-fade-in">
          <Zap size={10} /> Nouveau — Conseiller IA intégré
        </div>
        <h1 className="text-6xl font-semibold tracking-tight mb-6 leading-tight animate-slide-up">
          Anticipez vos ruptures<br />
          <span className="text-brand-400">avant qu'elles arrivent</span>
        </h1>
        <p className="text-zinc-400 text-xl max-w-2xl mx-auto mb-10 animate-fade-in">
          StockSense prédit avec précision les risques de rupture de stock grâce au ML — avec date estimée, jauge visuelle, alertes automatiques et conseiller IA.
        </p>
        <div className="flex items-center justify-center gap-4 animate-fade-in">
          <Link to="/register">
            <button className="btn-primary flex items-center gap-2 px-8 py-3 text-base">
              Démarrer gratuitement <ArrowRight size={16} />
            </button>
          </Link>
          <Link to="/login" className="text-zinc-400 hover:text-zinc-100 transition-colors">
            Déjà un compte →
          </Link>
        </div>
        <p className="text-xs text-zinc-600 mt-4">5 prédictions gratuites · Aucune carte bancaire</p>
      </section>

      {/* Stats */}
      <section className="border-y border-surface-border py-12">
        <div className="max-w-4xl mx-auto px-8 grid grid-cols-4 gap-8 text-center">
          {stats.map(({ value, label }) => (
            <div key={label}>
              <p className="text-3xl font-semibold text-brand-400 mb-1">{value}</p>
              <p className="text-sm text-zinc-500">{label}</p>
            </div>
          ))}
        </div>
      </section>

      {/* Features */}
      <section className="max-w-6xl mx-auto px-8 py-24">
        <h2 className="text-3xl font-semibold text-center mb-3">Tout ce qu'il vous faut</h2>
        <p className="text-zinc-500 text-center mb-14">Une plateforme complète pour gérer votre stock intelligemment</p>
        <div className="grid grid-cols-3 gap-5">
          {features.map(({ icon: Icon, title, desc, color }) => (
            <div key={title} className="card hover:border-zinc-600 transition-colors">
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center mb-4 ${color}`}>
                <Icon size={18} />
              </div>
              <h3 className="font-medium text-zinc-200 mb-2">{title}</h3>
              <p className="text-zinc-500 text-sm leading-relaxed">{desc}</p>
            </div>
          ))}
        </div>
      </section>

      {/* How it works */}
      <section className="border-t border-surface-border py-24">
        <div className="max-w-4xl mx-auto px-8">
          <h2 className="text-3xl font-semibold text-center mb-3">Comment ça marche</h2>
          <p className="text-zinc-500 text-center mb-14">Opérationnel en moins de 5 minutes</p>
          <div className="grid grid-cols-2 gap-6">
            {steps.map(({ n, title, desc }) => (
              <div key={n} className="card flex gap-4">
                <span className="text-3xl font-semibold text-brand-600/40 shrink-0">{n}</span>
                <div>
                  <h3 className="font-medium text-zinc-200 mb-1">{title}</h3>
                  <p className="text-zinc-500 text-sm">{desc}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Pricing */}
      <section className="border-t border-surface-border py-24">
        <div className="max-w-3xl mx-auto px-8">
          <h2 className="text-3xl font-semibold text-center mb-3">Tarifs simples</h2>
          <p className="text-zinc-500 text-center mb-14">Commencez gratuitement, évoluez quand vous êtes prêt</p>
          <div className="grid grid-cols-2 gap-6">
            {plans.map(plan => (
              <div key={plan.name} className={`card relative ${plan.highlight ? 'border-brand-600/60 bg-brand-500/5' : ''}`}>
                {plan.highlight && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-brand-600 text-white text-xs px-3 py-0.5 rounded-full font-medium">
                    Recommandé
                  </div>
                )}
                <h3 className="font-semibold text-zinc-100 mb-1">{plan.name}</h3>
                <div className="flex items-baseline gap-1 mb-6">
                  <span className="text-4xl font-semibold">{plan.price}</span>
                  <span className="text-zinc-500 text-sm">{plan.period}</span>
                </div>
                <ul className="space-y-2.5 mb-6">
                  {plan.features.map(f => (
                    <li key={f} className="flex items-center gap-2 text-sm text-zinc-400">
                      <CheckCircle size={13} className="text-brand-400 shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
                <Link to="/register">
                  <button className={`w-full py-2.5 rounded-lg text-sm font-medium transition-all ${plan.highlight ? 'btn-primary' : 'btn-ghost border border-surface-border'}`}>
                    {plan.cta}
                  </button>
                </Link>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="border-t border-surface-border py-24 text-center">
        <div className="max-w-xl mx-auto px-8">
          <h2 className="text-3xl font-semibold mb-4">Prêt à anticiper vos ruptures ?</h2>
          <p className="text-zinc-500 mb-8">Rejoignez StockSense et ne soyez plus jamais pris par surprise.</p>
          <Link to="/register">
            <button className="btn-primary px-8 py-3 text-base flex items-center gap-2 mx-auto">
              Commencer gratuitement <ArrowRight size={16} />
            </button>
          </Link>
        </div>
      </section>

      {/* Footer */}
      <footer className="border-t border-surface-border px-8 py-8 text-center text-zinc-600 text-sm">
        <div className="flex items-center justify-center gap-2 mb-2">
          <Activity size={14} className="text-brand-500" />
          <span className="font-medium text-zinc-500">StockSense</span>
        </div>
        <p>© 2026 StockSense · Prédiction de ruptures de stock par ML</p>
      </footer>
    </div>
  )
}
